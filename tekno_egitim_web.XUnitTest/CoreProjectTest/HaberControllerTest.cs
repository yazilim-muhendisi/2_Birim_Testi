...
namespace tekno_egitim_web.XUnitTest.CoreProjectTest
{
    ...
    public class HaberControllerTest
    {
        ...
        [Theory]
        [InlineData(1)]
        public async void Details_ValidIdReturnHaber(int haber_id)
        {
            Haber Haber = Habers.First(x => x.haber_id == haber_id);
            //_mock.Setup(repository => repository.GetByIdAsync());
            var result = await _controller.Details(haber_id);
            var viewresult = Assert.IsType<ViewResult>(result);
        ...
        [Theory]
        [InlineData(3)]
        public async void Edit_IdInvalid_ReturnHaberFound(int haber_id)
        {
            Haber Haber = null;
            _mock.Setup(x => x.GetByIdAsync(haber_id)).ReturnsAsync(Haber);
            var result = await _controller.Edit(haber_id);
            //var redirect = Assert.IsType<HaberFoundResult>(result);
            //Assert.Equal<int>(404, redirect.StatusCode);
        }
        ...
        [Theory]
        [InlineData(2)]
        public async void Edit_ActionExecutes_ReturnHaber(int haber_id)
        {
            var Haber = Habers.First(x => x.haber_id == haber_id);
            _mock.Setup(repository => repository.GetByIdAsync(haber_id)).ReturnsAsync(Haber);
            var result = await _controller.Edit(haber_id);
            var viewresult = Assert.IsType<ViewResult>(result);
            var resultHaber = Assert.IsAssignableFrom<Haber>(viewresult.Model);
            Assert.Equal(Haber.haber_id, resultHaber.haber_id);
        ...
        [Theory]
        [InlineData(1)]
        public void EditPost_IdIsHaberEqualProduct_ReturnHaberFound(int haber_id)
        {
            var result = _controller.Edit(2, Habers.First(x => x.haber_id == haber_id));
            //var redirect = Assert.IsType<HaberFoundResult>(result);
        ...
        [Theory]
        [InlineData(1)]
        public void EditPost_ValidModelState_ReturnRedirectToIndexAction(int haber_id)
        {
            var result = _controller.Edit(haber_id, Habers.First(x => x.haber_id == haber_id));
            var redirect = Assert.IsType<RedirectToActionResult>(result);
            Assert.Equal("Index", redirect.ActionName);
        }
        [Theory]
        [InlineData(1)]
        public async void EditPost_ValidModelState_UpdateMethodExecute(int haber_id)
        {
            var Haber = Habers.First(x => x.haber_id == haber_id);
            _mock.Setup(repository => repository.Update(Haber));
            await _controller.Edit(haber_id, Haber);
        ...
        [Theory]
        [InlineData(0)]
        public async void Delete_IdIsHaberNullEqualHaber_ReturnHaberFound(int haber_id)
        {
            Haber Haber = null;
            _mock.Setup(x => x.GetByIdAsync(haber_id)).ReturnsAsync(Haber);
            var result = await _controller.Delete(haber_id);
            //Assert.IsType<HaberFoundResult>(result);
        }

        [Theory]
        [InlineData(1)]
        public async void Delete_ActionExecute_ReturnHaber(int haber_id)
        {
            var Haber = Habers.First(x => x.haber_id == haber_id);
            _mock.Setup(repository => repository.GetByIdAsync(haber_id)).ReturnsAsync(Haber);
            var result = await _controller.Delete(haber_id);
        ...
        [Theory]
        [InlineData(1)]
        public async void DeleteConfirmed_ActionExecutes_ReturRedirectToIndexAction(int haber_id)
        {
            var result = await _controller.DeleteConfirmed(haber_id);
            Assert.IsType<RedirectToActionResult>(result);
        }

        [Theory]
        [InlineData(1)]
        public async void DeleteConfirmed_ActionExecutes_DeleteMethodExecute(int haber_id)
        {
            var Haber = Habers.First(x => x.haber_id == haber_id);
            //_mock.Setup(repository => repository.Delete(Haber));
            await _controller.DeleteConfirmed(haber_id);
